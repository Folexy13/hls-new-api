...
const cloudinary_utility_1 = require("../utilities/cloudinary.utility");
/**
 * @swagger
 * components:
 *   securitySchemes:
...
let SupplementController = class SupplementController extends base_controller_1.BaseController {
    ...
    ensurePrincipalRole(req, res) {
        if (req.user.role !== 'principal') {
            response_utility_1.ResponseUtil.error(res, 'Only principals can manage supplements', 403);
            return false;
        }
        return true;
    }
    ...
    async getUserSupplements(req, res) {
        try {
            if (!this.ensurePrincipalRole(req, res))
                return;
   ...
   */
    ...
    async uploadImage(req, res) {
        try {
            if (!this.ensurePrincipalRole(req, res))
                return;
            if (!req.file) {
                return response_utility_1.ResponseUtil.error(res, 'No image file provided', 400);
            }
            const result = await cloudinary_utility_1.cloudinaryService.uploadFile(req.file, 'supplements');
            return response_utility_1.ResponseUtil.success(res, {
                imageUrl: result.url,
            ...
            return response_utility_1.ResponseUtil.error(res, error.message);
        }
    }
    /**
     * @swagger
     ...
     *         description: Only principals can upload images
     */
    async uploadImageBase64(req, res) {
        try {
            if (!this.ensurePrincipalRole(req, res))
                return;
            const { image } = req.body;
            if (!image) {
                return response_utility_1.ResponseUtil.error(res, 'No image data provided', 400);
            }
            const result = await cloudinary_utility_1.cloudinaryService.uploadBase64(image, 'supplements');
            return response_utility_1.ResponseUtil.success(res, {
                imageUrl: result.url,
                publicId: result.publicId,
            }, 'Image uploaded successfully');
